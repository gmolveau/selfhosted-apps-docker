...
# $ sudo mkdir -p "$DEST"
# $ for v in bitwarden_data freshrss_data nextcloud_data nextcloud_db wallabag_data wallabag_images wekan_db wekan_db_dump; do bash volume_backup.sh backup "$v" "$DEST"; done
...
# $ for v in bitwarden_data freshrss_data nextcloud_data nextcloud_db wallabag_data wallabag_images wekan_db wekan_db_dump; do docker volume create "$v" && bash volume_backup.sh restore "$v" $(ls -Art "${DEST}/${v}"* | tail -n 1); done
